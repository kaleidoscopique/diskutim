...
int main (int ac, char *av[])
{
	...
	// Parsage de la DB users et indexation des lignes dans la hashtable
	...
	do
	{
		if(rand()%30 == 0) printf("Le serveur écoute.......\n");
		...
		if (sd_comm != -1)
		{
			...
			printf("Un nouveau client frappe à la porte : %d...\n",sd_comm);
			add_new_descriptor(&listof_connected_clients,sd_comm);

			// Affichage de la liste
			printf("Liste MAJ des clients connectés : ");
	...
	// Nettoyage de la mémoire (param du thread) + fermeture socket
	free(param);
	close(sd_serv);
